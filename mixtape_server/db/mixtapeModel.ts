...
let mixtapeModel = {
    ...
    getMixtape: async function getMixtape (id: number) {
        const collection : Collection = db.collection("mixtapes");
    ...
    deleteMixtapeByID: async function deleteMixtapeByID(id: number) {
        const collection = db.collection("mixtapes");
    ...
    createMixtape: async function createMixtape(mixtapeData: {title: string; tracks: string[], author: number}) {
        const collection  = db.collection("mixtapes");
        ...
        try {
            ...
            if (result.acknowledged) {
                return { success: true, data: mixtapeData };
            } else {
                return { success: false, data: mixtapeData };
            }
        } catch (error) {
